...
function countAssignmentShapes(content: string | undefined): number {
  ...
  return content
    .split("\n")
    .filter((line) => /^(?:(?:export|typeset(?:\s+-[A-Za-z]+)+)\s+)?[A-Za-z_][A-Za-z0-9_]*=/.test(line.trim()))
    .length;
}
...
export class EnvironmentProbes {
  constructor(private readonly dependencies: EnvironmentProbeDependencies = defaultDependencies()) {}
  ...
  collect(options: EnvironmentProbeOptions): EnvironmentSnapshot {
    ...
    const xdgConfig = options.xdgConfig ?? this.dependencies.environment.XDG_CONFIG_HOME ?? join(home, ".config");
    const xdgCache = options.xdgCache ?? this.dependencies.environment.XDG_CACHE_HOME ?? join(home, ".cache");
    ...
    if (privateMetadata === undefined && privateContent !== undefined) failures.push("private-file");

    const privateState: PrivateFileMetadata = privateMetadata === undefined
      ? { kind: "absent", assignmentCount: 0 }
      : {
          kind: privateMetadata.kind === "file" || privateMetadata.kind === "symlink" ? privateMetadata.kind : "other",
    ...
    const commands = (["node", "pnpm", "java"] as const).map((name) => this.safeCommand(name, failures));
    const pnpm = commands.find((command) => command.name === "pnpm") ?? { name: "pnpm", status: "absent" as const };
    const pnpmGlobalBin = pnpm.status === "present"
      ? this.safePnpmGlobalBinDirectory(failures)
      : { status: "absent" as const };
    const javaHome = this.dependencies.platform === "darwin"
    ...
    return {
      ...
      pathEntries,
      zshTopology: usesSourceAll(zshrcContent) ? "source-all" : zshrcContent === undefined ? "unknown" : "modular",
      currentLink,
      ...
      pnpm: {
        ...
        globalBinDiscoverable:
          pnpmGlobalBin.status === "present" && pathEntries.some((entry) => entry.path === pnpmGlobalBin.directory),
      },
  ...
  private safeRead(probe: "zsh-topology" | "private-file", path: string, failures: AuditProbeName[]): string | undefined {
    try {
