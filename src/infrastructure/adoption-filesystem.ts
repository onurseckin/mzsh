import { chmodSync, closeSync, constants, fstatSync, lstatSync, mkdirSync, openSync, readFileSync, readlinkSync, realpathSync, renameSync, rmSync, symlinkSync, unlinkSync, writeSync } from "node:fs";
...
export class NodeAdoptionFilesystem {
  ...
  describe(path: string): AdoptionTargetState {
    try {
      ...
      if (stat.isFile()) return { path, kind: "file", mode, ownerId: stat.uid, hash: this.hash(path) };
      if (stat.isSymbolicLink()) return { path, kind: "symlink", mode, ownerId: stat.uid, linkTarget: readlinkSync(path) };
  ...
  hasSafeOwnedRoot(path: string): boolean {
    ...
    if (state.kind !== "directory" || state.ownerId === undefined || userId === undefined || state.ownerId !== userId) return false;
