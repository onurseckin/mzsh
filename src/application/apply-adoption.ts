...
import type { AdoptionApplyResult, AdoptionMutation, AdoptionReceipt, AdoptionReceiptTarget, AdoptionTargetState } from "../domain/adoption";
...
export interface ApplyAdoptionDependencies {
  ...
  preflight(plan: AdoptionPlan): { kind: "passed" } | { kind: "failed"; code: "syntax-invalid" | "isolated-startup-failed" };
...
export function applyAdoption(plan: AdoptionPlan, dependencies: ApplyAdoptionDependencies): AdoptionApplyResult {
  ...
  try {
    if (
      ...
      filesystem.hash(join(plan.repository, "package.json")) !== plan.repositoryPreconditions.packageHash
    ) {
      return { kind: "failed", stage: "preflight", code: "repository-changed", path: plan.repository };
    }
  } catch {
    return { kind: "failed", stage: "preflight", code: "repository-changed", path: plan.repository };
  }
  let preflight: { kind: "passed" } | { kind: "failed"; code: "syntax-invalid" | "isolated-startup-failed" };
  try {
    preflight = dependencies.preflight(plan);
  } catch {
    return { kind: "failed", stage: "preflight", code: "preflight-unavailable", path: plan.entrypoint };
  }
  if (preflight.kind === "failed") return { kind: "failed", stage: "preflight", code: preflight.code, path: plan.entrypoint };
  ...
  if (plan.privateMigration !== undefined) {
    try {
      ...
      const sourceTarget = plan.targets.find((target) => target.path === plan.privateMigration?.sourcePath);
      if (
        sourceTarget === undefined ||
        legacySnapshot.state.hash !== plan.privateMigration.sourceHash ||
        !sameState(sourceTarget.before, legacySnapshot.state)
      ) {
        return { kind: "failed", stage: "preflight", code: "source-changed", path: plan.privateMigration.sourcePath };
      }
    } catch {
      return { kind: "failed", stage: "preflight", code: "source-changed", path: plan.privateMigration.sourcePath };
    }
  ...
  try {
    ...
    for (const mutation of plan.mutations) {
      ...
      if (mutation.category === "legacy" && plan.privateMigration !== undefined) {
        if (legacySnapshot === undefined || !sameState(legacySnapshot.state, filesystem.describe(plan.privateMigration.sourcePath))) {
      ...
      if (mutation.kind === "symlink" && mutation.linkTarget !== undefined) {
        ...
        const privateLines = plan.privateMigration === undefined
          ? ""
          : plan.privateMigration.selectedLineIndexes.map((index) => legacySnapshot?.text.split(/(?<=\n)/)[index] ?? "").join("");
        filesystem.writeAtomic(mutation.path, `${existingPrivate}${existingPrivate.length > 0 && !existingPrivate.endsWith("\n") && privateLines.length > 0 ? "\n" : ""}${privateLines}`);
        ...
        filesystem.writeAtomic(mutation.path, lines.filter((_line, index) => !plan.privateMigration!.selectedLineIndexes.includes(index)).join(""));
      }
    ...
    const targets: AdoptionReceiptTarget[] = plan.targets.map((target, index) => ({
      ...
      ...(backupPaths.has(target.path) ? { backupRelativePath: join("backups", `${index}-${basename(target.path)}`) } : {}),
    }));
    const receipt: AdoptionReceipt = {
      ...
      repository: { root: plan.repository, version: plan.repositoryMetadata.version, commit: plan.repositoryMetadata.commit },
