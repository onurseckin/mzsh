...
const targetKinds: readonly AdoptionTargetKind[] = [
  'absent',
  'file',
  'symlink',
  'directory',
...
function isState(value: unknown): value is AdoptionTargetState {
  if (
    ...
    !targetKinds.includes(value.kind as AdoptionTargetKind)
  )
    return false;
  if (value.mode !== undefined && (typeof value.mode !== 'number' || !Number.isInteger(value.mode)))
    return false;
  if (
    value.ownerId !== undefined &&
    (typeof value.ownerId !== 'number' || !Number.isInteger(value.ownerId))
  )
    return false;
  if (
...
function isReceipt(value: unknown): value is AdoptionReceipt {
  return (
    ...
    value.pathOrder.every(
      (item) => typeof item === 'string' && categories.includes(item as (typeof categories)[number])
    ) &&
...
function isSafeBackupPath(
  stateDirectory: string,
  backupRelativePath: string,
  filesystem: NodeAdoptionFilesystem
): boolean {
  if (isAbsolute(backupRelativePath) || backupRelativePath.split(/[\\/]/).includes('..'))
    return false;
  return filesystem.isContainedWithoutEscape(
    stateDirectory,
    resolve(stateDirectory, backupRelativePath)
  );
}
...
function validateReceipt(
  ...
  if (![receipt.home, receipt.config, receipt.stateDirectory].every((path) => isAbsolute(path)))
    return false;
  if (
    !filesystem.hasSafeOwnedRoot(receipt.home) ||
    !filesystem.hasSafeOwnedRoot(receipt.config) ||
    !filesystem.hasSafeOwnedRoot(receipt.stateDirectory)
  )
    return false;
  ...
  if (
    ...
    receiptState.mode !== 0o600 ||
    receiptState.ownerId === undefined ||
    filesystem.currentUserId() === undefined ||
    receiptState.ownerId !== filesystem.currentUserId()
  )
    return false;
  ...
  const loaderPaths = new Set([
    join(receipt.home, '.zshenv'),
    join(receipt.home, '.zprofile'),
    join(receipt.home, '.zshrc'),
  ]);
  ...
  if (
    loaderPaths.size !== 0 ||
    required.size !== 0 ||
    receipt.targets.filter((target) => target.category === 'legacy').length > 1
  )
    return false;
  ...
  return receipt.targets.every((target) => {
    ...
    const root =
      target.category === 'loader' || target.category === 'legacy' ? receipt.home : receipt.config;
    ...
    const validLoader = [
      ...
      join(receipt.home, '.zprofile'),
      join(receipt.home, '.zshrc'),
    ].includes(target.original.path);
    if (
      (target.category === 'loader' && !validLoader) ||
      (expectedPath !== undefined && target.original.path !== expectedPath)
    )
      return false;
    if (
      ...
      return false;
    if (
      (target.category === 'shims' || target.category === 'current') &&
      target.applied.kind !== 'symlink'
    )
      return false;
    const backupValid = (() => {
      if (target.original.kind !== 'file') return target.backupRelativePath === undefined;
      if (target.backupRelativePath === undefined || target.original.hash === undefined)
        return false;
      if (!isSafeBackupPath(receipt.stateDirectory, target.backupRelativePath, filesystem))
        return false;
      ...
      return (
        ...
        filesystem.currentUserId() !== undefined &&
        state.ownerId === filesystem.currentUserId() &&
        state.hash === target.original.hash
      );
    })();
    return (
...
export function rollbackAdoption(
  ...
  if (receipt === undefined)
    return { kind: 'failed', code: 'receipt-invalid', path: input.receiptPath };
  ...
  try {
    ...
    filesystem.writeAtomic(
      input.receiptPath,
      JSON.stringify({ ...receipt, status: 'unavailable' }),
      0o600
    );
    ...
    for (const target of [
      ...journal.filter((target) => target.category !== 'current'),
      ...journal.filter((target) => target.category === 'current'),
    ]) {
      try {
...
function restoreOriginal(
  filesystem: NodeAdoptionFilesystem,
  receipt: AdoptionReceipt,
  target: AdoptionReceiptTarget
): void {
  const backupPath =
    target.backupRelativePath === undefined
      ? undefined
      : join(receipt.stateDirectory, target.backupRelativePath);
