...
export function auditEnvironment(snapshot: EnvironmentSnapshot): EnvironmentAuditReport {
  ...
  if (repository.kind === "missing") {
    findings.push(
      finding(
        "REPOSITORY_MISSING",
        "warning",
        "The configured MZSH repository root is absent.",
        { kind: "repair", summary: "Provide a valid local MZSH checkout before installation work." }
      )
    );
  ...
  if (snapshot.pathEntries.some((entry) => entry.mode !== undefined && (entry.mode & 0o022) !== 0)) {
    findings.push(
  ...
  if (snapshot.zshTopology === "source-all") {
    findings.push(
      finding("ZSH_SOURCE_ALL_TOPOLOGY", "warning", "Live Zsh configuration sources files as an unordered set.", {
        kind: "configure",
  ...
  if (privateFile.kind === "absent") {
    findings.push(
      finding("PRIVATE_FILE_ABSENT", "info", "No local private override file is present.", inspectRemediation)
    );
    ...
    findings.push(
      finding("PRIVATE_FILE_NON_REGULAR", "error", "The local private override is not a regular file.", {
        kind: "repair",
    ...
    if (privateFile.ownerId === undefined || privateFile.currentUserId === undefined) {
      findings.push(
        finding("PRIVATE_FILE_INSECURE", "error", "The local private override ownership could not be verified.", {
          kind: "repair",
      ...
      findings.push(
        finding("PRIVATE_FILE_FOREIGN_OWNER", "error", "The local private override has a foreign owner.", {
          kind: "repair",
      ...
      findings.push(
        finding("PRIVATE_FILE_INSECURE", "error", "The local private override permissions are too broad.", {
          kind: "repair",
      ...
      findings.push(
        finding("PRIVATE_FILE_SECURE", "info", "A local private override has owner-only permissions.", inspectRemediation)
      );
    }
  }
  ...
  if (snapshot.nodeOwnership.nvmInteractive && snapshot.nodeOwnership.homebrewPrivateNode) {
    findings.push(
      finding("NODE_OWNERSHIP_CONFLICT", "warning", "NVM and Homebrew-private Node ownership overlap.", {
        kind: "configure",
  ...
  if (snapshot.pnpm.status === "present" && !snapshot.pnpm.globalBinDiscoverable) {
    findings.push(
      finding("PNPM_GLOBAL_BIN_UNDISCOVERABLE", "warning", "The pnpm global-bin directory is not discoverable on PATH.", {
        kind: "configure",
  ...
  if (snapshot.java.status === "not-discovered") {
    findings.push(
      finding("JAVA_NOT_DISCOVERED", "info", "macOS Java home discovery did not find a registered JDK.", inspectRemediation)
    );
  }
  ...
  for (const command of snapshot.commands) {
    if (command.status === "absent") {
      findings.push(
        finding("OPTIONAL_TOOL_ABSENT", "info", `Optional tool ${command.name} is absent.`, inspectRemediation)
      );
    }
  }
