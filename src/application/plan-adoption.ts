...
function planAdoptionUnchecked(
  input: PlanAdoptionInput,
  dependencies: PlanAdoptionDependencies
): AdoptionPlanResult {
  ...
  if (!filesystem.hasSafeOwnedRoot(input.config))
    return rejected('config-root-unsafe', input.config);
  if (!filesystem.hasSafeOwnedRoot(input.repository))
    return rejected('repository-root-unsafe', input.repository);
  if (!filesystem.isContainedWithoutEscape(input.home, input.config))
    return rejected('config-outside-home', input.config);
  if (
    input.legacySource !== undefined &&
    !filesystem.isContainedWithoutEscape(input.home, input.legacySource)
  ) {
  ...
  if (filesystem.describe(entrypoint).kind !== 'file')
    return rejected('portable-entrypoint-missing', entrypoint);
  ...
  if (new Set(targets).size !== targets.length)
    return rejected('legacy-source-overlaps-target', input.legacySource ?? input.home);
  ...
  for (const target of targets) {
    ...
    const root =
      ...
      [
        ...
        ? input.home
        : input.config;
    if (!filesystem.isContainedWithoutEscape(root, target))
      return rejected('target-outside-root', target);
    if (
      [
        join(input.home, '.zshenv'),
        join(input.home, '.zprofile'),
      ...
      if (state.kind !== 'absent' && state.kind !== 'file')
        return rejected('loader-destination-unsafe', target);
      if (
        state.kind === 'file' &&
  ...
  if (
    privateBefore.kind === 'symlink' ||
    privateBefore.kind === 'other' ||
    privateBefore.kind === 'directory'
  ) {
  ...
  if (
    ...
    (privateBefore.ownerId === undefined ||
      filesystem.currentUserId() === undefined ||
      privateBefore.ownerId !== filesystem.currentUserId())
  ) {
  ...
  if (input.legacySource !== undefined) {
    ...
    if (source?.kind !== 'file' || source.hash === undefined)
    ...
    const selectedLineIndexes = lines.flatMap((line, index) =>
      dependencies.isSensitiveAssignment?.(line) === true ? [index] : []
    );
  ...
  try {
    ...
    if (
      ...
      metadata === null ||
      (metadata as { name?: unknown }).name !== 'mzsh'
    ) {
  ...
  if (
    filesystem.describe(stateDirectory).kind !== 'absent' ||
    filesystem.describe(join(stateDirectory, 'receipt.json')).kind !== 'absent'
  ) {
  ...
  const plan: AdoptionPlan = {
    ...
    mutations: [
      ...
      ...(privateMigration?.selectedLineIndexes.length
        ? [{ category: 'legacy' as const, path: input.legacySource!, kind: 'file' as const }]
        : []),
      {
        category: 'shims',
        path: join(managedRoot, 'shims'),
        kind: 'symlink',
        linkTarget: join(input.repository, 'portable', 'zsh', 'shims'),
      },
      {
        category: 'current',
        path: join(managedRoot, 'current'),
        kind: 'symlink',
        linkTarget: join(input.repository, 'portable', 'zsh'),
      },
    ],
  ...
  if (
    !targetObjects.every((target) => sameState(target.before, filesystem.describe(target.path)))
  ) {
