...
export function parseArguments(args: readonly string[]): ParsedArguments {
  ...
  if (command === undefined) return { kind: 'unmanaged' };
  if (!['audit', 'bootstrap', 'update', 'rollback'].includes(command))
  ...
  for (let index = 1; index < args.length; index += 1) {
    ...
    if (token === '--apply') {
      if (apply || command === 'audit') return { kind: 'usage-error', code: 'invalid-flags' };
      apply = true;
      continue;
    }
    if (token === '--json') {
      if (json || command !== 'audit') return { kind: 'usage-error', code: 'invalid-flags' };
      json = true;
      continue;
    }
    if (token === '--source' || token === '--legacy-source') {
      if (token === '--source' && command === 'rollback')
        return { kind: 'usage-error', code: 'invalid-flags' };
      const next = args[index + 1];
      if (next === undefined || next.startsWith('-') || !isAbsolute(next))
        return { kind: 'usage-error', code: 'absolute-path-required' };
      if (token === '--source') {
        if (source !== undefined) return { kind: 'usage-error', code: 'duplicate-flag' };
        source = next;
      } else {
        if (command !== 'bootstrap') return { kind: 'usage-error', code: 'invalid-flags' };
        if (legacySource !== undefined) return { kind: 'usage-error', code: 'duplicate-flag' };
        legacySource = next;
      }
      index += 1;
      continue;
    ...
    if (command !== 'rollback' || receiptId !== undefined)
      return { kind: 'usage-error', code: 'unexpected-positional' };
    receiptId = token;
  }
  if (command === 'audit')
    return { kind: 'audit', ...(source === undefined ? {} : { source }), json };
  ...
  if (command === 'update')
    return { kind: 'update', ...(source === undefined ? {} : { source }), apply };
