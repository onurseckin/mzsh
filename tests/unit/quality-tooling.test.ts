...
} from '../../scripts/check/check-typescript-line-limits';
...
test('routes JavaScript and shell quality checks through compatible compiled tools', () => {
  ...
  expect(packageJson.scripts['shell:check']).toContain('scripts/check/check-shell-quality.sh');
  ...
  expect(existsSync(join(root, 'scripts', 'check', 'check-shell-quality.sh'))).toBe(true);
  ...
  const shellGate = readFileSync(join(root, 'scripts', 'check', 'check-shell-quality.sh'), 'utf8');
...
test('keeps every regular TypeScript source and test file within the physical line limit', () => {
  expect(packageJson.scripts['line:check']).toBe(
    'bun scripts/check/check-typescript-line-limits.ts'
  );
  expect(packageJson.scripts['quality:check']).toContain('bun run line:check');
  expect(TYPE_SCRIPT_DIRECTORIES).toEqual(['scripts', 'src', 'tests']);
  expect(ROOT_TYPE_SCRIPT_FILES).toEqual([]);
