...
function plan(repository: string): AdoptionPlan {
  return { schema: "mzsh.adoption-plan/v1", id: "test", home: "/isolated", repository, config: "/isolated/.config", stateDirectory: "/isolated/state", backupDirectory: "/isolated/backups", privatePath: "/isolated/private", currentLink: "/isolated/current", shimLink: "/isolated/shims", entrypoint: join(repository, "portable", "zsh", "init.zsh"), repositoryPreconditions: { entrypointHash: "", packageHash: "" }, moduleOrder: [], targets: [], mutations: [], repositoryMetadata: { version: "test", commit: null } };
}
...
describe("stable loader rendering", () => {
  ...
  test("syntax-checks an isolated portable checkout and fails closed for invalid syntax", () => {
    ...
    expect(new ZshPreflight().preflight(plan(root))).toEqual({ kind: "failed", code: "syntax-invalid" });
  });
  ...
  test("uses only generated-home paths, fixed source arguments, and cleans up on success", () => {
    ...
    const dependencies: ZshPreflightDependencies = {
      ...
      removeTemporaryHome: (path) => { removed = path === isolated; rmSync(path, { recursive: true, force: true }); },
      spawn: (_command, args, environment) => { calls.push({ args, environment }); return { status: 0 }; },
    };
    expect(new ZshPreflight(dependencies).preflight(plan(root))).toEqual({ kind: "passed" });
    expect(calls).toHaveLength(7);
    expect(calls.every((call) => call.environment.HOME === isolated && call.environment.ZDOTDIR === isolated && call.environment.PATH === "/usr/bin:/bin:/usr/sbin:/sbin")).toBe(true);
    expect(calls.slice(-3).every((call) => call.args.includes('source "$1"') && !call.args.some((arg) => arg.includes("~")))).toBe(true);
  ...
  test("fails closed and removes the generated home when command output resembles a credential assignment", () => {
    ...
    expect(new ZshPreflight(dependencies).preflight(plan(root))).toEqual({ kind: "failed", code: "syntax-invalid" });
    expect(removed).toBe(true);
  });

  test("fails closed for compound credential assignments in stdout or stderr", () => {
    for (const result of [{ status: 0, stdout: "DB_PASSWORD_HASH=inert-placeholder" }, { status: 0, stderr: "SERVICE_CREDENTIALS=inert-placeholder" }]) {
      const root = mkdtempSync(join(tmpdir(), "mzsh-preflight-test-"));
      fixtures.push(root);
      mkdirSync(join(root, "portable", "zsh"), { recursive: true });
      writeFileSync(join(root, "portable", "zsh", "init.zsh"), "return 0\n");
      const isolated = mkdtempSync(join(tmpdir(), "mzsh-preflight-isolated-"));
      const dependencies: ZshPreflightDependencies = { createTemporaryHome: () => isolated, removeTemporaryHome: (path) => rmSync(path, { recursive: true, force: true }), spawn: () => result };
      expect(new ZshPreflight(dependencies).preflight(plan(root))).toEqual({ kind: "failed", code: "syntax-invalid" });
    }
  });
  ...
  test("fails closed without cleanup when generated-home creation fails", () => {
    let cleanupCalls = 0;
    const dependencies: ZshPreflightDependencies = {
      createTemporaryHome: () => { throw new Error("injected"); },
      removeTemporaryHome: () => { cleanupCalls += 1; },
      spawn: () => ({ status: 0 }),
    };
    expect(new ZshPreflight(dependencies).preflight(plan("/unread"))).toEqual({ kind: "failed", code: "isolated-startup-failed" });
  ...
  test("fails closed when generated-home cleanup fails", () => {
    ...
    const dependencies: ZshPreflightDependencies = {
      createTemporaryHome: () => isolated,
      removeTemporaryHome: () => { throw new Error("injected"); },
      spawn: () => ({ status: 0 }),
    };
    expect(new ZshPreflight(dependencies).preflight(plan(root))).toEqual({ kind: "failed", code: "isolated-startup-failed" });
  });
});
