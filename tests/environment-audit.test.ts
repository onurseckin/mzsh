...
describe("environment audit", () => {
  test("reports repository, PATH, topology, link, ownership, and optional-tool findings without secrets", () => {
    const report = auditEnvironment(
      snapshot({
        ...
        commands: [{ name: "node", status: "present", executablePath: "/isolated/bin/node", version: "v22.0.0" }],
  ...
  test("classifies private file absence, secure files, insecure permissions, and foreign ownership", () => {
    expect(codes(auditEnvironment(snapshot({ privateFile: { kind: "absent", assignmentCount: 0 } })))).toContain(
      "PRIVATE_FILE_ABSENT"
    );
    expect(
      codes(auditEnvironment(snapshot({ privateFile: { kind: "file", mode: 0o600, ownerId: 501, currentUserId: 501, assignmentCount: 2 } })))
    ).toContain("PRIVATE_FILE_SECURE");
    expect(
      codes(auditEnvironment(snapshot({ privateFile: { kind: "file", mode: 0o644, ownerId: 501, currentUserId: 501, assignmentCount: 2 } })))
    ).toContain("PRIVATE_FILE_INSECURE");
    expect(
      codes(auditEnvironment(snapshot({ privateFile: { kind: "file", mode: 0o600, ownerId: 502, currentUserId: 501, assignmentCount: 2 } })))
    ).toContain("PRIVATE_FILE_FOREIGN_OWNER");
    expect(
      codes(auditEnvironment(snapshot({ privateFile: { kind: "other", mode: 0o700, ownerId: 501, currentUserId: 501, assignmentCount: 0 } })))
    ).toContain("PRIVATE_FILE_NON_REGULAR");
    expect(
      codes(auditEnvironment(snapshot({ privateFile: { kind: "file", mode: 0o600, assignmentCount: 2 } })))
    ).toContain("PRIVATE_FILE_INSECURE");
  });
  ...
  test("keeps absent optional commands informational and redacts probe failures to their probe names", () => {
    ...
    expect(failedProbe).toEqual({
      ...
      remediation: { kind: "inspect", summary: "Review the named probe locally and rerun the audit." },
    });
  });
  ...
  test("detects source-all loops while leaving explicit manifest sourcing modular", () => {
    ...
    expect(probes.collect({ repositoryRoot: "/isolated/repository", zshrc: "/isolated/.zshrc" }).zshTopology).toBe(
      "source-all"
    );
    expect(
  ...
  test("uses the adopted private target, counts exported assignment shapes, and checks the configured pnpm bin directory", () => {
    const dependencies: EnvironmentProbeDependencies = {
      ...
      currentUserId: 501,
      inspectPath: (path) => {
        if (path === "/isolated/home/.config/mzsh/private.zsh") return { kind: "file", mode: 0o600, ownerId: 501 };
        if (path === "/isolated/bin") return { kind: "directory", mode: 0o755 };
        if (path === "/isolated/home/Library/pnpm") return { kind: "directory", mode: 0o755 };
        return undefined;
      },
      readText: (path) => {
        if (path === "/isolated/.zshrc") return 'source "$HOME/.config/zsh/"*.zsh\nsource "$NVM_DIR/nvm.sh"\n';
  ...
  test("keeps runnable Java separate from macOS JDK discovery", () => {
    const java = commandMetadataFromVersionResult("java", "/usr/bin/java", 1, "java stub diagnostic");
    const runnableJava = commandMetadataFromVersionResult("java", "/opt/homebrew/bin/java", 0, "17.0.1");
    const report = auditEnvironment(
