...
function runInteractive(root: string, script: string, extraEnvironment: Record<string, string> = {}): ReturnType<typeof Bun.spawnSync> {
...
describe("portable interactive behavior modules", () => {
  test("keeps interactive-only definitions out of a noninteractive shell", () => {
    const root = fixture();
    const result = Bun.spawnSync([zshPath, "-df", "-c", [
  ...
  test("defines prompt, aliases, search, and history behavior quietly only in an interactive shell", () => {
    ...
    expect(output(result)).toContain("ALIASES=rmtrash:nvim \"$HOME/Library/Application Support/tealdeer/config.toml\"\n");
  ...
  test("keeps framework loading optional while preparing vi configuration before a present framework", () => {
    ...
    writeFileSync(join(framework, "themes", "powerlevel10k", "powerlevel10k.zsh-theme"), "return 0\n");
    writeFileSync(join(framework, "oh-my-zsh.sh"), 'print -r -- "FRAMEWORK=$ZVM_INIT_MODE:$ZSH_THEME:${plugins[(I)zsh-vi-mode]}"\n');

    const present = runInteractive(root, [
      `source ${JSON.stringify(join(modulesRoot, "oh-my-zsh.zsh"))}`,
      'print -r -- "LOADED=${MZSH_OH_MY_ZSH_LOADED:-absent}"',
    ].join("\n"));
    const absent = runInteractive(fixture(), `source ${JSON.stringify(join(modulesRoot, "oh-my-zsh.zsh"))}`);
  ...
  test("redacts database passwords and performs no port action until validated helpers are called", () => {
    ...
    writeFileSync(join(bin, "lsof"), `#!/bin/zsh\nprint -r -- "$*" >> ${JSON.stringify(calls)}\nprint -r -- 4242\n`);
