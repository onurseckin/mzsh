...
describe('portable runtime paths and safety shims', () => {
  test('ships executable runners and the exact managed link topology', () => {
    for (const name of ['check-prohibited', 'shim-runner'])
      expect(lstatSync(join(shims, name)).mode & 0o111).not.toBe(0);
    for (const name of [
      'bun',
      'bunx',
      'dd',
  ...
  test('delegates allowed commands and refuses recursive protected-target removal using only fakes', () => {
    ...
    const delegated = Bun.spawnSync([join(copiedShims, 'npm'), '--version'], {
      env: environment,
      stdout: 'pipe',
      stderr: 'pipe',
    });
    const refused = Bun.spawnSync([join(copiedShims, 'rm'), '-rf', home], {
      env: environment,
      stdout: 'pipe',
      stderr: 'pipe',
    });
  ...
  test('removes recursive shim aliases before command resolution and returns 127 when no real command exists', () => {
    ...
    const delegated = Bun.spawnSync([join(copiedShims, 'npm'), 'one value', 'two'], {
      env: environment,
      stdout: 'pipe',
      stderr: 'pipe',
    });
    const missing = Bun.spawnSync([join(copiedShims, 'bun')], {
      env: environment,
      stdout: 'pipe',
      stderr: 'pipe',
    });
  ...
  test('refuses every guarded destructive command family with fakes only', () => {
    ...
    for (const [command, ...arguments_] of refusals) {
      const result = Bun.spawnSync([join(copiedShims, command), ...arguments_], {
        env: environment,
        stdout: 'pipe',
        stderr: 'pipe',
      });
